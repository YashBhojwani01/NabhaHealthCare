...
const SignupDoctor: React.FC = () => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!agree) return;
    localStorage.setItem('auth_user', JSON.stringify({ ...form, role: 'doctor' }));
    navigate('/doctor');
  };
        ...
        <Card className="shadow-elevated">
          ...
          <CardContent>
            <form className="space-y-4" onSubmit={onSubmit}>
              ...
              <div>
                <Label htmlFor="registrationNo">Registration No (PMC/NMC)</Label>
                <Input id="registrationNo" name="registrationNo" value={form.registrationNo} onChange={onChange} required />
              </div>
              <div>
